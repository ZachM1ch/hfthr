...
<body>
<h1>Heigh's Flooring and Tile House Repository - Inventory Search Results</h1>
<?php

 // variables
 $searchtype=$_POST['searchtype'];
 $searchterm=trim($_POST['searchterm']);
 // check if vars are filled
...
alt="'.$imagepath.'" />';
 ...
 echo '<input type="submit" name="delete_item" value="Delete Product">';
 $sku = $row['sku'];
 echo'<input type="hidden" name="prod_sku" value="'.$sku.'">';
 echo '</form>';

 echo '<form action="modify.php" method="post">';
 echo '<input type="submit" name="modify_item" value="Modify Product">';
 $sku = $row['sku'];
 echo'<input type="hidden" name="prod_sku" value="'.$sku.'">';;
 echo '</form>';
 echo '<br />';
